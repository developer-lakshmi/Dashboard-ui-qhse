    ...
    },
    {
      ...
      component: (
        <div className="flex flex-col items-center w-full">
          ...
          <ResponsiveContainer width="100%" height={Math.max(400, carsObsData.length * 36)}>
            <BarChart
              layout="vertical"
              data={carsObsData}
              margin={{ top: 16, right: 24, left: 120, bottom: 40 }}
              barCategoryGap="10%"
              barSize={18} // Makes bars thicker
            >
              <CartesianGrid strokeDasharray="3 3" />
              <YAxis
                dataKey="name"
        ...
        <div className="flex flex-col w-full">
          <div className="space-y-3 max-h-96 overflow-y-auto w-full">
            {timelineData.map((project, index) => (
              <div
                key={index}
                className="flex flex-col gap-1 p-4 bg-gradient-to-r from-blue-50/60 via-white to-green-50/60 dark:from-gray-900 dark:via-gray-800 dark:to-green-900 rounded-xl border border-gray-100 dark:border-gray-800 shadow-sm"
              >
                <div className="flex justify-between items-center mb-1">
                  <span className="font-semibold text-gray-800 dark:text-gray-100 text-base truncate">{project.name}</span>
